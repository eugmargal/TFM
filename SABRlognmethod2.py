...
def SABRBlfuncobj(param,FS,K,sigmaMKT,Expiry,beta,s):
    ...
    for j in range(len(K)):
        if  K[j] == FS:
            ...
            Aprima = 1 + ((1-beta)*logFK[j])**2/24 + ((1-beta)*logFK[j])**4/1920 
            Bprima = (alpha(param[0],param[1])*(1-beta))**2/(24*KFS[j]**2) + 0.25*alpha(param[0],param[1])*param[0]*param[1]*beta/KFS[j] + (2-3*param[0])**2*param[1]**2/24
            c = param[1]*KFS[j]*logFK[j]/alpha(param[0],param[1])
            gprima = (math.sqrt(c**2 - 2*param[1]*c + 1) + c - param[0])/(1-param[0])
 
            vol2 = alpha(param[0],param[1])*(1+Bprima*Expiry)*c/(KFS[j]*Aprima*math.log(gprima))
