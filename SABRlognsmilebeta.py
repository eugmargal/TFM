...
def SABRBlplot(param,FS,K,Expiry,s):
  ...
  estimatedvol = list with the estimated vols for every K in the introduced array
    ...
    for j in range(len(K)):
        if  K[j] == FS:
            ...
            Aprima = 1 + ((1-param[3])*logFK[j])**2/24 + ((1-param[3])*logFK[j])**4/1920
            Bprima = (param[0]*(1-param[3]))**2/(24*KFS[j]**2) + 0.25*param[0]*param[1]*param[2]*param[3]/KFS[j] + (2-3*param[1]**2)*param[2]**2/24
...
def SABRBlfuncobj(param,FS,K,sigmaMKT,Expiry,s):
...
def SABRBlcalb(FS,K,sigmaMKT,Expiry,s):
    """ Returns SABR volatilies for each strike desired 
